...
const schema = z.object({
  project: z.object({ name: z.string(), instructions: z.string().max(20_000).optional() }).optional(),
  task: z.object({ goal: z.string().max(20_000), status: z.string().optional() }).optional(),
  messages: z.array(item).max(200).default([]),
  memories: z.array(item).max(200).default([]),
  maxChars: z.number().int().min(2_000).max(200_000).default(60_000)
});
...
function rank(items: z.infer<typeof item>[]): z.infer<typeof item>[] { return [...items].sort((a, b) => (b.importance ?? 0.5) - (a.importance ?? 0.5)); }

export function buildContext(input: ContextInput): ContextResult {
  const max = input.maxChars;
  ...
  for (const memory of rank(input.memories)) { if (!add(`MEMORY\n${memory.key ?? ""}: ${memory.content}`, memory.id, "memory")) break; }
...
export async function registerContextRoutes(app: FastifyInstance): Promise<void> {
  app.post("/api/v1/context/build", { preHandler: authenticate }, async request => {
