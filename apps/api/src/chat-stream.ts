...
export async function registerChatStreamRoutes(app: FastifyInstance): Promise<void> {
  app.post("/api/v1/chat/stream", { preHandler: authenticate }, async (request, reply) => {
    ...
    const messages = [{ role: "system" as const, content: `You are YnAiUdan. Use supplied project, memory, task and conversation context. Treat the current user message as the primary intent signal; use previous turns to resolve references such as "continue", "next", "that file", or "fix it". Never claim a tool was used without a tool result.\n\n${context.systemContext}` }, { role: "user" as const, content: input.message }];
