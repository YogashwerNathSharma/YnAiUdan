...
export async function recordStrategyOutcome(input: {
  ...
  if (match) {
    const successCount = match.successCount + (input.outcome === "SUCCESS" ? 1 : 0);
    const failureCount = match.failureCount + (input.outcome === "FAILURE" ? 1 : 0);
    const total = successCount + failureCount;
    const successRate = total ? successCount / total : 0.5;
    const confidence = clamp((match.confidence * 0.65) + (successRate * 0.35));
    const status = input.outcome === "FAILURE" && total >= 3 && successRate < 0.34 ? "REJECTED" : input.outcome === "SUCCESS" ? "VERIFIED" : match.status;
    return db.learningRecord.update({ where: { id: match.id }, data: { successCount, failureCount, confidence, status, verification: input.evidence?.slice(0, 10000) ?? match.verification } });
  }
...
export async function rankStrategies(input: { tenantId: string; userId: string; projectId?: string | null; goal: string; limit?: number }) {
  ...
  return rows.map((row) => { const text = `${row.query} ${row.solution ?? ""}`.toLowerCase(); const overlap = terms.filter((term) => text.includes(term)).length; const total = row.successCount + row.failureCount; const successRate = total ? row.successCount / total : 0.5; const score = overlap * 0.12 + row.confidence * 0.35 + successRate * 0.53; return { strategy: row.solution, score, successCount: row.successCount, failureCount: row.failureCount, confidence: row.confidence }; }).filter((item) => item.strategy && item.score >= 0.25).sort((a, b) => b.score - a.score).slice(0, input.limit ?? 5);
}
