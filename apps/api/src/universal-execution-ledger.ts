...
export function createExecutionLedger(taskId: string, tenantId: string): ExecutionLedger {
  ...
  const ledger: ExecutionLedger = { taskId, tenantId, events: [], createdAt: new Date().toISOString() };
  ledgers.set(taskId, ledger);
  return ledger;
}

export function appendExecutionEvidence(input: Omit<ExecutionEvidence, "id" | "timestamp" | "integrityHash">): ExecutionEvidence {
  const ledger = createExecutionLedger(input.taskId, input.tenantId);
  const event: Omit<ExecutionEvidence, "integrityHash"> = { ...input, id: randomUUID(), timestamp: new Date().toISOString() };
  const evidence: ExecutionEvidence = { ...event, integrityHash: hashEvent(event) };
  ledger.events.push(evidence);
  return evidence;
}

export function getExecutionLedger(taskId: string, tenantId: string): ExecutionLedger {
  const ledger = ledgers.get(taskId);
  if (!ledger || ledger.tenantId !== tenantId) throw new Error("Execution ledger not found");
  return ledger;
}
...
export function verifyExecutionLedger(ledger: ExecutionLedger): { valid: boolean; checked: number; invalidEventIds: string[] } {
  ...
  for (const event of ledger.events) {
    ...
    if (hashEvent(payload) !== integrityHash) invalidEventIds.push(event.id);
  }
