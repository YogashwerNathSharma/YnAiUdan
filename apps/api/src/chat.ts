...
export async function registerChatRoutes(app: FastifyInstance): Promise<void> {
  app.post("/api/v1/chat", { preHandler: authenticate }, async (request, reply) => {
    ...
    return reply.send({ message: { id: assistant.id, role: assistant.role, content: assistant.content, createdAt: assistant.createdAt }, provider: provider.name, model: result.model, context: { characters: context.characters, truncated: context.truncated, memories: context.selectedMemoryIds.length } });
  });
}
