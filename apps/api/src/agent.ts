...
export async function registerAgentRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/api/v1/agents/tasks/:id/plan", { preHandler: authenticate }, async (request, reply) => {
    const auth = request.user as AuthPayload; const { id } = request.params as { id: string }; const task = await db.task.findFirst({ where: { id, tenantId: auth.tenantId, userId: auth.sub } });
    if (!task) return reply.code(404).send({ error: "Task not found" });
    try {
      const steps = await planToolSteps(task.goal, task.model);
      if (steps.length > (task.maxSteps ?? 50)) return reply.code(422).send({ error: "Generated plan exceeds task step limit" });
      ...
      const updated = await db.task.update({ where: { id: task.id }, data: { status: "WAITING_APPROVAL" }, include: { steps: { orderBy: { sequence: "asc" } } } });
      return reply.send({ task: updated, toolPlan: steps });
