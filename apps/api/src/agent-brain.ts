...
export async function runAgentBrain(taskId: string, options: BrainRunOptions) {
  ...
  for (let cycle = 1; cycle <= maxCycles; cycle += 1) {
    ...
    if (result.status === "COMPLETED") {
      ...
      if (refreshed?.status === "COMPLETED") { const evaluation = evaluateAgentRun({ goal: refreshed.goal, completed: true, verified: verifiedTaskResult(result), recoveryCount: replans, repeatedFailure: false, toolErrors: events.filter(event => event.phase === "EXECUTE" && (event.result as Record<string, unknown> | undefined)?.status === "FAILED").length, evidenceCount: completedEvidenceCount(refreshed) }); events.push({ cycle, phase: "EVALUATE", evaluation }); return { status: "COMPLETED", cycles: cycle, events, evaluation }; }
    }
