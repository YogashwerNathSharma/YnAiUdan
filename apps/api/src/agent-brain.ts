...
function readyParallelBatch(steps: PlannedStep[], maxBatch = 4): PlannedStep[] {
  ...
  const firstInput = first.input && typeof first.input === "object" ? first.input as Record<string, unknown> : {};
  const group = typeof firstInput.parallelGroup === "string" ? firstInput.parallelGroup : undefined;
  ...
  const ready = pending.filter(step => {
    ...
    if (input.parallelGroup !== group) return false;
    const dependencies = Array.isArray(input.dependsOn) ? input.dependsOn.filter((value): value is number => Number.isInteger(value) && value > 0) : [];
    return dependencies.every(sequence => completedSequences.has(sequence));
  });
  return ready.slice(0, maxBatch);
}
...
export async function runAgentBrain(taskId: string, options: BrainRunOptions) {
  ...
  for (let cycle = 1; cycle <= maxCycles; cycle += 1) {
    ...
    const batch = readyParallelBatch(task.steps, 4);
    if (batch.length === 0) return { status: "PAUSED", reason: "DEPENDENCY_DEADLOCK", cycles: cycle, events };
    await appendExecutionEvidence({ taskId: task.id, tenantId: options.tenantId, kind: "TOOL_REQUESTED", actor: "AGENT", summary: `Execution cycle ${cycle} started${batch.length > 1 ? ` with ${batch.length} parallel steps` : ""}`, data: { cycle, stepIds: batch.map(step => step.id), tools: batch.map(step => (step.input as Record<string, unknown> | null)?.toolName), parallel: batch.length > 1 } });
    const results = await Promise.all(batch.map(step => executeNextTaskStep(task.id, options.userId, options.tenantId, options.role, step.id)));
    for (let index = 0; index < results.length; index += 1) {
      ...
      await appendExecutionEvidence({ taskId: task.id, tenantId: options.tenantId, kind: resultStatus === "FAILED" ? "TASK_FAILED" : "EXECUTION_FINISHED", actor: "AGENT", summary: `Step ${batch[index].id} finished with ${resultStatus}`, data: { cycle, stepId: batch[index].id, status: resultStatus, verified: verifiedTaskResult(result), parallelBatchSize: batch.length } });
    }
    const failedResult = results.find(result => result.status === "FAILED");
    const approvalResult = results.find(result => result.status === "WAITING_APPROVAL");
    if (approvalResult) return { status: "WAITING_APPROVAL", cycles: cycle, events };
    if (failedResult) {
      diagnosticEvidence = diagnosticFromResult(failedResult); recoveryStrategyHints = strategyHints(failedResult);
      const fingerprint = failureFingerprint(failedResult);
