...
export async function runAgentBrain(taskId: string, options: BrainRunOptions) {
  const maxCycles = Math.min(100, Math.max(1, options.maxCycles ?? 20)); const events: Array<Record<string, unknown>> = []; let replans = 0; let diagnosticEvidence: string | undefined; const failureFingerprints = new Set<string>();
  for (let cycle = 1; cycle <= maxCycles; cycle += 1) {
    ...
    if (task.status === "PLANNING") {
      const steps = await planToolSteps(task.goal, task.model ?? "mock:default", { userId: options.userId, tenantId: options.tenantId, projectId: task.projectId ?? undefined, diagnosticEvidence }); if (steps.length > (task.maxSteps ?? 50)) throw new Error("Generated plan exceeds task step limit");
      ...
      events.push({ cycle, phase: "PLAN", steps: steps.length, approvalRequired: true, usedDiagnosticEvidence: Boolean(diagnosticEvidence), distinctFailureCount: failureFingerprints.size }); return { status: "WAITING_APPROVAL", cycles: cycle, events };
    }
    ...
    if (result.status === "FAILED") {
      diagnosticEvidence = diagnosticFromResult(result); const fingerprint = failureFingerprint(result);
      if (fingerprint && failureFingerprints.has(fingerprint)) { await db.task.update({ where: { id: task.id }, data: { status: "PAUSED" } }); events.push({ cycle, phase: "REPAIR_GUARD", reason: "REPEATED_FAILURE", fingerprint }); return { status: "PAUSED", reason: "REPEATED_FAILURE", cycles: cycle, events, replans, diagnosticEvidence: Boolean(diagnosticEvidence) }; }
      if (fingerprint) failureFingerprints.add(fingerprint);
      if (!options.replanOnFailure || replans >= Math.max(0, task.maxRetries ?? 0)) return { status: "FAILED", cycles: cycle, events, replans, diagnosticEvidence: Boolean(diagnosticEvidence) };
      replans += 1; await db.task.update({ where: { id: task.id }, data: { status: "PLANNING" } }); events.push({ cycle, phase: "REPLAN", attempt: replans, reason: result.error ?? "step failed", hasDiagnosticEvidence: Boolean(diagnosticEvidence), failureFingerprint: fingerprint });
    }
  }
