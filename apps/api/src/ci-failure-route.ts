...
export async function registerCiFailureRoutes(app: FastifyInstance): Promise<void> {
  app.post("/api/v1/ci/analyze", { preHandler: authenticate }, async (request, reply) => {
    ...
    return reply.send({ tenantId: auth.tenantId, analysis: analyzeCiFailure(input) });
  });
}
