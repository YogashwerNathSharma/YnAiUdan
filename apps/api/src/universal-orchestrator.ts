...
import { buildAutonomousFactoryPlan, type AutonomousFactoryPlan } from "./autonomous-factory.js";
...
export type UniversalOrchestrationResult = {
  ...
  status: string;
  plan: UniversalPlan;
  factoryPlan: AutonomousFactoryPlan;
  brain?: unknown;
...
function canAutoExecute(mode: AutonomyMode, approvalGranted: boolean): boolean {
  return approvalGranted && (mode === "AUTONOMOUS" || mode === "FULLY_CONTROLLED");
}
...
export async function orchestrateUniversalGoal(input: UniversalOrchestrationInput): Promise<UniversalOrchestrationResult> {
  ...
  const plan = buildUniversalPlan(goal, platform);
  const factoryPlan = buildAutonomousFactoryPlan(goal, platform);
      ...
      steps: {
        create: {
          ...
          input: {
            goal,
            universalPlan: plan,
            autonomousFactoryPlan: factoryPlan,
          },
        },
      },
    },
  ...
  await appendExecutionEvidence({
    ...
    kind: "GOAL_ACCEPTED",
    actor: "USER",
    summary: "Universal goal accepted by the autonomous factory",
    data: { platform, model, autonomyMode, factoryVersion: factoryPlan.version },
  });
  await appendExecutionEvidence({
    taskId: task.id,
    tenantId: input.tenantId,
    kind: "PLAN_CREATED",
    actor: "SYSTEM",
    summary: "Universal and factory execution plans created",
    data: {
      workItems: plan.workItems,
      executionWaves: plan.executionWaves,
      factoryTargets: factoryPlan.targets,
      factoryWaves: factoryPlan.waves,
      qualityGates: factoryPlan.qualityGates,
      approvalGates: factoryPlan.approvalGates,
    },
  });
  for (const item of plan.workItems) {
    await appendExecutionEvidence({
      ...
      kind: "AGENT_ASSIGNED",
      actor: "SYSTEM",
      summary: `${item.agent} assigned to ${item.workItemId}`,
      data: { risk: item.risk, dependsOn: item.dependsOn ?? [], parallelGroup: item.parallelGroup },
    });
  }
  ...
  const brainStatus =
    firstBrain && typeof firstBrain === "object"
      ? String((firstBrain as Record<string, unknown>).status ?? "PLANNING")
      : "PLANNING";
  ...
  if (approvalRequired) {
    await appendExecutionEvidence({
      ...
      data: { autonomyMode, approvalGates: factoryPlan.approvalGates },
    });
  }
  ...
  const pendingSteps = await db.taskStep.findMany({
  ...
  await Promise.all(
    pendingSteps.map(step => {
      ...
      return db.taskStep.update({
        where: { id: step.id },
        data: { input: { ...stepInput, approvalGranted: true } },
      });
    }),
  );
  ...
  await appendExecutionEvidence({
    ...
    actor: "USER",
    summary: "Execution approval granted",
    data: { approvedSteps: pendingSteps.length },
  });
  ...
  await appendExecutionEvidence({
    ...
    actor: "SYSTEM",
    summary: "Universal task queued for autonomous execution",
    data: { queue: "taskQueue", steps: pendingSteps.length },
  });
