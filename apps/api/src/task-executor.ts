...
export async function executeNextTaskStep(taskId: string, userId: string, tenantId: string, role: string) {
  ...
  if (result.ok) {
    ...
    if (!verified) { const reason = input.toolName === "github.ci_status" ? "CI did not reach SUCCESS" : "Repository write completed without verified read-back evidence"; const verification = verificationSummary(input.toolName, result.output); await db.taskStep.update({ where: { id: step.id }, data: { status: "FAILED", error: reason, completedAt: new Date(), output: { result: result.output, verification } } }); await db.task.update({ where: { id: task.id }, data: { status: "FAILED" } }); await persistExecutionMemory(task, input as { toolName: string; input?: unknown }, { ok: false, tool: input.toolName, error: reason }); return { status: "FAILED", stepId: step.id, tool: input.toolName, error: reason, verification }; }
    const verification = verificationSummary(input.toolName, result.output); await db.taskStep.update({ where: { id: step.id }, data: { status: "COMPLETED", output: { result: result.output, verification }, completedAt: new Date() } }); await persistExecutionMemory(task, input as { toolName: string; input?: unknown }, result); return { status: "COMPLETED", stepId: step.id, tool: result.tool, output: result.output, verification };
  }
