...
function compactMemoryValue(value: unknown): string {
  try { const text = typeof value === "string" ? value : JSON.stringify(value); return text.slice(0, 9000); } catch { return String(value).slice(0, 9000); }
}

function normalizeAutonomyMode(mode: string): AutonomyMode {
  if (mode === "SUGGEST_ACTIONS") return "SUGGEST";
  if (mode === "ASK_BEFORE_TOOLS") return "CONFIRM_TOOLS";
  if (mode === "AUTO_SAFE") return "SAFE_AUTO";
  if (["CHAT_ONLY", "SUGGEST", "CONFIRM_TOOLS", "SAFE_AUTO", "AUTONOMOUS", "FULLY_CONTROLLED"].includes(mode)) return mode as AutonomyMode;
...
export async function executeNextTaskStep(taskId: string, userId: string, tenantId: string, role: string) {
  ...
  if (!step) { await db.task.update({ where: { id: task.id }, data: { status: "COMPLETED" } }); return { status: "COMPLETED" }; }
  if (step.name !== "TOOL") { await db.taskStep.update({ where: { id: step.id }, data: { status: "COMPLETED", startedAt: new Date(), completedAt: new Date(), output: { acknowledged: true } } }); return { status: "COMPLETED", stepId: step.id, next: true }; }
  ...
  const result = await executeTool({ toolName: input.toolName, input: input.input, role, mode: normalizeAutonomyMode(task.autonomyMode), context: { tenantId: task.tenantId, userId: task.userId, projectId: task.projectId ?? undefined } });
  ...
  if (result.requiresApproval) { await db.taskStep.update({ where: { id: step.id }, data: { status: "PENDING" } }); await db.task.update({ where: { id: task.id }, data: { status: "WAITING_APPROVAL" } }); return { status: "WAITING_APPROVAL", stepId: step.id, tool: result.tool, error: result.error }; }
