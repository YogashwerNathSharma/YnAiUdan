...
function verifiedGitHubWrite(toolName: string, output: unknown): boolean { if (!["github.commit", "github.push"].includes(toolName)) return true; if (!output || typeof output !== "object" || !("evidence" in output)) return false; const evidence = (output as { evidence?: unknown }).evidence; return Boolean(evidence && typeof evidence === "object" && "verified" in evidence && (evidence as { verified?: unknown }).verified === true); }
...
export async function executeNextTaskStep(taskId: string, userId: string, tenantId: string, role: string) {
  ...
  if (result.ok) {
    if (!verifiedGitHubWrite(input.toolName, result.output)) { await db.taskStep.update({ where: { id: step.id }, data: { status: "FAILED", error: "Repository write completed without verified read-back evidence", completedAt: new Date() } }); await db.task.update({ where: { id: task.id }, data: { status: "FAILED" } }); await persistExecutionMemory(task, input as { toolName: string; input?: unknown }, { ok: false, tool: input.toolName, error: "Missing or failed repository write verification" }); return { status: "FAILED", stepId: step.id, tool: input.toolName, error: "Repository write was not verified" }; }
    await db.taskStep.update({ where: { id: step.id }, data: { status: "COMPLETED", output: result.output as object, completedAt: new Date() } }); await persistExecutionMemory(task, input as { toolName: string; input?: unknown }, result); return { status: "COMPLETED", stepId: step.id, tool: result.tool, output: result.output };
  }
