...
export class GitHubHttpClient implements GitHubClient {
  ...
  async listDirectory(repo: GitHubRepository, path = "", ref?: string): Promise<unknown[]> { const suffix = ref ? `?ref=${encodeURIComponent(ref)}` : ""; const encodedPath = path ? `/${path.split("/").map(encodeURIComponent).join("/")}` : ""; const data = (await this.request(`/repos/${encodeURIComponent(repo.owner)}/${encodeURIComponent(repo.name)}/contents${encodedPath}${suffix}`)).data; return Array.isArray(data) ? data : []; }
