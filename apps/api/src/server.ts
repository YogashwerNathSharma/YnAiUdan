...
import { registerAuthRoutes, authenticate } from "./auth.js";
...
app.setErrorHandler((error, request, reply) => {
  if (error.name === "ZodError" || error.validation) return reply.code(400).send({ error: "Invalid request" });
...
app.get("/api/v1/tools", { preHandler: authenticate }, async () => toolRegistry.list());
app.get("/api/v1/models", { preHandler: authenticate }, async () => modelRouterInfo());
app.get("/api/v1/metrics", { preHandler: authenticate }, async () => runtimeMetrics());
app.get("/api/v1/provider-health", { preHandler: authenticate }, async () => providerHealthSnapshot());
