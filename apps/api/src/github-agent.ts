...
export async function registerGitHubAgentRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/api/v1/github/branch", { preHandler: authenticate }, async (request, reply) => { const input = repoSchema.extend({ branch: z.string().regex(/^[A-Za-z0-9._/-]+$/).max(200), fromRef: z.string().max(200).optional() }).parse(request.body); const auth = request.user as AuthPayload; if (!["OWNER", "ADMIN", "DEVELOPER"].includes(auth.role)) return reply.code(403).send({ error: "GitHub branch creation requires developer permissions" }); try { return reply.code(201).send(await githubRegistry.getClient().createBranch(input, input.branch, input.fromRef)); } catch (error) { return reply.code(503).send({ error: error instanceof Error ? error.message : "GitHub integration unavailable" }); } });
  app.post("/api/v1/github/commit", { preHandler: authenticate }, async (request, reply) => { const input = repoSchema.extend({ branch: z.string().min(1).max(200), message: z.string().min(1).max(500), changes: changesSchema }).parse(request.body); try { return reply.send(await githubRegistry.getClient().commitChanges(input, input.branch, input.message, input.changes)); } catch (error) { return reply.code(503).send({ error: error instanceof Error ? error.message : "GitHub commit failed" }); } });
  app.post("/api/v1/github/push", { preHandler: authenticate }, async (request, reply) => { const input = repoSchema.extend({ branch: z.string().min(1).max(200), message: z.string().min(1).max(500), changes: changesSchema }).parse(request.body); try { return reply.send(await githubRegistry.getClient().push(input, input.branch, input.message, input.changes)); } catch (error) { return reply.code(503).send({ error: error instanceof Error ? error.message : "GitHub push failed" }); } });
  app.post("/api/v1/github/pr", { preHandler: authenticate }, async (request, reply) => { const input = repoSchema.extend({ title: z.string().min(1).max(300), head: z.string().min(1).max(200), base: z.string().min(1).max(200), body: z.string().max(20_000).optional() }).parse(request.body); try { return reply.code(201).send(await githubRegistry.getClient().createPullRequest(input, input.title, input.head, input.base, input.body)); } catch (error) { return reply.code(503).send({ error: error instanceof Error ? error.message : "Pull request creation failed" }); } });
}
