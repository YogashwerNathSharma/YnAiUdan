...
export async function registerGitHubAgentRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/api/v1/github/tenant-status", { preHandler: authenticate }, async request => { const auth = request.user as AuthPayload; return { tenantId: auth.tenantId, userId: auth.sub, role: auth.role, configured: githubRegistry.configured() }; });
}
