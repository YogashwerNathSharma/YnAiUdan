...
export function registerCodingTools(): void {
  toolRegistry.register({
    ...
    description: "Write a UTF-8 file inside the YnAiUdan workspace sandbox.",
    ...
    timeoutMs: 10_000,
    execute: async input => { await writeWorkspaceFile(input.path, input.content); return { path: input.path, bytes: Buffer.byteLength(input.content, "utf8") }; }
  });
}
...
export async function registerCodingRoutes(app: FastifyInstance): Promise<void> {
  app.post("/api/v1/coding/command", { preHandler: authenticate }, async (request, reply) => {
    const auth = request.user as { role: string };
    const input = commandSchema.parse(request.body);
    const result = await executeTool({ toolName: "terminal.execute", input, role: auth.role, mode: input.mode });
